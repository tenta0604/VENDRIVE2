...
function inputSchema(){return {type:"object",additionalProperties:false,properties:{paper:paperSchema(),totalQty:{type:"integer",minimum:0},items:{type:"array",items:inputLineSchema()}},required:["paper","totalQty","items"]}}
...
async function handle(request){
  ...
  const prompt=[
    ...
    "For any field that is genuinely uncertain or partially legible, add a warning token beginning uncertain: followed by its field path, for example uncertain:items.2.productCode, uncertain:input.items.1.quantity, uncertain:vendorNumber, or uncertain:occurredAt. Do not mark clearly legible fields as uncertain.",
