...
async function handle(request) {
  ...
  const system = [
    ...
    "相手が回答期限を指定していても、その期限内の最終回答をこちらが確約できる事実が入力にない場合は、draft_replyで期限内回答を約束しないでください。希望期限は受領したとだけ述べ、対応可否・金額・回答時刻は確認後に連絡する表現にしてください。",
    "next_actionでは、期限が迫っている場合は『期限までに最終回答』を断定せず、『期限までに回答可能か至急確認し、難しい場合は期限前に一次連絡する』方針を優先してください。",
    "draft_replyでは、入力に実際の着手・確認開始・調整開始が明記されていない限り、『現在確認中です』『調整を進めています』『社内確認を行っています』など進行中の状態を示す表現を使わないでください。代わりに『確認のうえご連絡します』『対応可否と金額を確認し、改めてご連絡します』のように未来の行動として表現してください。",
    "簡潔な日本語で出力してください。"
  ].join("\n");
