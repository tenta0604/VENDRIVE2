...
- Visible app candidate: **2026.09.18-FINAL.18** (bounded OCR visit-flow maintenance; Analytics engine **AN14B3B3**, DB/schema remain 4).
...
Real field use on 2026-09-18 produced bounded maintenance corrections rather than a new speculative feature phase. FINAL.16 hardened OCR period/identity correction. The FINAL.17 maintenance release added explicit post-confirm visit closeout and sales-period overlap protection. Real-device use then exposed a narrower FINAL.18 maintenance issue: a compact-review rerender could drop visit callbacks, especially after correcting a sales-journal field, which removed the post-confirm continuation/end choices. FINAL.18 preserves those callbacks and connects explicit OCR visit closeout to the existing Today completion flow, including the existing task and order confirmations. Preview regression gates pass; production merge/live verification remains the release gate.
