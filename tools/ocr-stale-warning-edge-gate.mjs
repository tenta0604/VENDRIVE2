...
try{
  ...
  const zero=await ev(c,`(()=>{const review={classification:{effectiveType:"input_recovery",detectedType:"input_recovery",typeConfidence:1},identity:{occurredAt:"2026-09-15T08:38:00+09:00",makerKey:"suntory",vendorNumber:"4038404",machineId:"M1"},ocr:{warnings:["recovery_case_conversion_required"]},candidatePayload:{input:{paper:{locationName:"loc",operatorName:"op"},totalQty:1,items:[{productCode:"705904",printedName:"input",temperature:"COLD",column:null,quantity:1}]},recovery:{paper:{locationName:"loc",operatorName:"op"},totalQty:11,items:[{productCode:"744654",printedName:"yogurt",temperature:"COLD",column:null,caseCount:0,looseCount:10,quantity:10},{productCode:"744533",printedName:"ginger",temperature:"COLD",column:null,caseCount:0,looseCount:1,quantity:1}]}},machineMatch:{status:"confirmed_mapping",confirmedMapping:{machineId:"M1",missing:false},candidates:[]}};const host=document.getElementById("review");VENDRIVE2CompactOcrReview.mount(host,review);return{txt:host.innerText,width:innerWidth,scrollWidth:document.documentElement.scrollWidth}})()`);
