...
process.env.OCR_PROVIDER_ATTEMPT_TIMEOUT_MS="160";
process.env.OCR_PROVIDER_BUDGET_MS="600";
...
globalThis.fetch=(url,options)=>new Promise((resolve,reject)=>{
  ...
  const timer=setTimeout(()=>resolve(successResponse()),300);
...
assert.ok(elapsed<150,"hedged fallback waited too long");
...
assert.equal(response.status,200);
assert.ok(fastElapsed<40,"fast provider failure did not trigger immediate fallback");
